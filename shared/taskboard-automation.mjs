...
export function buildTaskboardAutomationPrompt(request) {
  return request.taskboardMode === "local"
...
function buildLocalTaskboardAutomationPrompt(request) {
  ...
  return [
    ...
    "从返回的 todo 中只选择依赖已完成的议题：relations.blockedBy 为空，或其中每个依赖的 status 都严格等于 done。无依赖的 todo 仍可并行处理。存在多个符合条件的 todo 时，必须选择 createdAt 最早的一个；createdAt 相同时按 identifier 升序选择。不得因标题、描述详略或列表位置让较新的议题插队。",
    "每次仅处理一个符合依赖条件的 todo：选定后先用 issue get 读取最新议题内容，并用 comment list <议题编号> --json 读取全部评论。根据描述和最新评论判断是否允许开始；若其中写明等待、暂不执行或当前不应开始，立即跳过并报告，不改状态。评论也包含已完成后被打回的返工要求。",
...
export function taskboardAutomationPolicyOperation(request, {
  ...
  if (taskboardMode === "local" && request.codexProjectKind !== "remote") {
